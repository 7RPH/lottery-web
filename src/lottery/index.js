...
function showExportOptions() {
  ...
  title.style.marginBottom = '25px'; // 增加标题下方间距
  title.style.fontSize = '20px'; // 增大标题字体
  ...
  btnContainer.style.marginBottom = '15px'; // 增加底部间距
  
  // Excel导出按钮
  const excelBtn = document.createElement('button');
  excelBtn.className = 'upload-btn';
  excelBtn.textContent = '导出Excel文件';
  excelBtn.style.marginRight = '15px';
  excelBtn.style.flex = '1';
  excelBtn.style.padding = '10px 15px'; // 增加按钮内边距
  ...
  pdfBtn.style.flex = '1';
  pdfBtn.style.padding = '10px 15px'; // 增加按钮内边距
  ...
  setTimeout(() => {
    ...
    // 计算新位置，只向上移动100px左右
    const newTop = buttonRect.top - 170; // 向上移动150px
    const newLeft = buttonRect.left - 150; // 确保中心对齐

    modalContent.style.top = `${Math.max(80, newTop)}px`; // 不要太靠近顶部
    modalContent.style.left = `${newLeft}px`;
    modalContent.style.width = '30vw';
    modalContent.style.height = '15vh'; // 增加高度
